...
public class Orders extends javax.swing.JPanel {
    ...
    private String deletePro, deletedPro, notDeletePro, cannotDeletePro, timePro, selectPro;
    private String notPro, updatedPro, notUpdatePro, cannotUpdatePro, selectInfoPro, existPro, addedPro, notAddPro, cannotAddPro;
    private String selectCus, addedOrder, notAddOrder, updatedOrder, notUpdateOrder, selectOrder, deleteOrder, deletedOrder, notDeleteOrder;
    private String id, staffName, discount, status, proid, orderDateError, thisItem, productLeft, quantityError, enoughQuantity, buy, more, notification;
    ...
    private void translate() {
        ...
        if (nn.equals(vn)) {
            ...
            try {
                ...
                jLabel16.setText(resource.getString("ordermanage"));
                jLabel4.setText(resource.getString("logout"));
                jLabel7.setText(resource.getString("cusname"));
                jLabel9.setText(resource.getString("orderdate"));
                jLabel8.setText(resource.getString("shipper"));
                jLabel14.setText(resource.getString("payment"));
                jLabel19.setText(resource.getString("sub"));
                jLabel15.setText(resource.getString("promotion"));
                jLabel17.setText(resource.getString("grand"));
                jLabel10.setText(resource.getString("proname"));
                jLabel13.setText(resource.getString("price"));
                jLabel12.setText(resource.getString("quantity"));
                jLabel11.setText(resource.getString("amount"));
                btnAddOrder.setText(resource.getString("order"));
                btnUpdateOrder.setText(resource.getString("order"));
                btnDeleteOrder.setText(resource.getString("order"));
                btnReset.setText(resource.getString("reset"));
                btnAddProduct.setText(resource.getString("product"));
                btnUpdateProduct.setText(resource.getString("product"));
                btnDeleteProduct.setText(resource.getString("product"));
                btnInvoice.setText(resource.getString("invoice"));
                btnSearchCustomer.setText(resource.getString("search"));
                btnSearchProduct.setText(resource.getString("search"));
                id = resource.getString("orderid");
                staffName = resource.getString("staffname");
                discount = resource.getString("discount");
                status = resource.getString("status");
                proid = resource.getString("proid");
                Border border = BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), resource.getString("detail"), TitledBorder.LEFT, TitledBorder.TOP, FONT_SIZE_14_BOLD, Color.white);
                jPanel9.setBorder(border);
                orderDateError = resource.getString("orderDateError");
                thisItem = resource.getString("thisItem");
                productLeft = resource.getString("productLeft");
                quantityError = resource.getString("quantityError");
                enoughQuantity = resource.getString("enoughQuantity");
                buy = resource.getString("buy");
                more = resource.getString("more");
                notification = resource.getString("notification");
                deletePro = resource.getString("deletePro");
                deletedPro = resource.getString("deletedPro");
                notDeletePro = resource.getString("notDeletePro");
                cannotDeletePro = resource.getString("cannotDeletePro");
                timePro = resource.getString("timePro");
                selectPro = resource.getString("selectPro");
                notPro = resource.getString("notPro");
                updatedPro = resource.getString("updatedPro");
                notUpdatePro = resource.getString("notUpdatePro");
                cannotUpdatePro = resource.getString("cannotUpdatePro");
                selectInfoPro = resource.getString("selectInfoPro");
                existPro = resource.getString("existPro");
                addedPro = resource.getString("addedPro");
                notAddPro = resource.getString("notAddPro");
                cannotAddPro = resource.getString("cannotAddPro");
                selectCus = resource.getString("selectCus");
                addedOrder = resource.getString("addedOrder");
                notAddOrder = resource.getString("notAddOrder");
                updatedOrder = resource.getString("updatedOrder");
                notUpdateOrder = resource.getString("notUpdateOrder");
                selectOrder = resource.getString("selectOrder");
                deleteOrder = resource.getString("deleteOrder");
                deletedOrder = resource.getString("deletedOrder");
                notDeleteOrder = resource.getString("notDeleteOrder");
            } catch (Exception e) {
            }
        }
        if (nn.equals(uk)) {
    ...
    private ArrayList search() {
        ...
        for (int i = 0; i < arr.size(); i++) {
            String id = ((Order) arr.get(i)).getOrderID().trim();
            ...
            if (id.contains(txtSearch.getText().toUpperCase()) || date.contains(txtSearch.getText().toUpperCase()) || name.contains(txtSearch.getText().toUpperCase()) || staff.contains(txtSearch.getText().toUpperCase())) {
    ...
    private void getOrders(ArrayList arr) {
        tblOrders.removeAll();
        Object[] obj = new Object[]{id, jLabel7.getText(), staffName, jLabel9.getText(), jLabel8.getText(), jLabel19.getText(), discount, jLabel14.getText(), status};
    ...
    private void getOrderDetails() {
        orderDetail = null;
        Object[] obj = new Object[]{id, proid, jLabel10.getText(), jLabel13.getText(), jLabel12.getText(), jLabel11.getText()};
...
//        dcrOrderDate.setEnabled(false);
    ...
    private void loadProductInfo() {
        ...
        lblStatus.setText(thisItem + " " + product.getQuantity() + " " + productLeft);
    }
    ...
    private boolean addProduct() {
        ...
        if (checkProductQuantity()) {
            if (chkPromotion.isSelected()) {
                ...
                if (!checkPromotion("insert").equals("PM006")) {
                    ...
                    if (!checkPromotionDate().equals("")) {
                        lblPromotionStatus.setText(buy + " " + currency.format(Dao_Promotion.search(checkPromotionDate()).getAmountOfMoney() - order.getTotalAmount()) + " " + more);
                    } else {
                        lblPromotionStatus.setText("");
                    }
                }
            ...
            Common.notify(enoughQuantity, notification, JOptionPane.WARNING_MESSAGE);
        }
        return valid;
    }
    ...
    private boolean add() {
        ...
        o = Dao_Orders.updateOrderIF(order.getOrderID(), order.getPromotionID(), intoMoney(), intoMoney() - intoMoney() * Dao_Promotion.search(order.getPromotionID()).getDiscount());
        if (od && p && o) {
            valid = true;
        }
        ...
        if (status.equals("insert")) {
            ...
            for (int i = 0; i < arr.size(); i++) {
                if (((Promotion) arr.get(i)).getPromotionDate().compareTo(orderDate) == 0 && ((Promotion) arr.get(i)).getQuantity() > 0 && (order.getTotalAmount() + Integer.parseInt(txtProductQuantity.getText().trim()) * product.getSellPrice()) > ((Promotion) arr.get(i)).getAmountOfMoney()) {
                    ...
                    Dao_Orders.updateOrderIF(order.getOrderID(), promotionID, intoMoney(), intoMoney() - intoMoney() * Dao_Promotion.search(order.getPromotionID()).getDiscount());
                    order = Dao_Orders.getOrder(order.getOrderID());
                    chkPromotion.setSelected(true);
    ...
    private void invoice(String orderID) {
        try {
            ...
            JasperReport report;
            report = JasperCompileManager.compileReport("src/com/view/Bill.jrxml");

            map.put("OrderID", orderID);

            JasperPrint p = JasperFillManager.fillReport(report, map, Dao_Login.conn);
    ...
    private void initComponents() {
        ...
        btnUpdateOrder.setText("Hóa Đơn");
        ...
        btnDeleteOrder.setText("Hóa Đơn");
        ...
        btnAddProduct.setText("Sản Phẩm");
        ...
        btnUpdateProduct.setText("Sản Phẩm");
        ...
        btnDeleteProduct.setText("Sản Phẩm");
        ...
        btnInvoice.setText("Xuất Hóa Đơn");
        ...
        if (order != null) {
            ...
            if (orderDetail != null) {
                if (date.plusDays(3).compareTo(today) == 0 || date.plusDays(3).isAfter(today)) {
                    int click = JOptionPane.showConfirmDialog(null, deletePro, notification, 2);
                    if (click == JOptionPane.YES_OPTION) {
                        boolean b = deleteProduct();
                        if (b) {
                            Common.notify(deletedPro, notification, 1);
                            refreshProduct();
                        } else {
                            Common.notify(notDeletePro, notification, JOptionPane.ERROR_MESSAGE);
                        }
                    }
                } else {
                    if (chkPromotion.isSelected()) {
                        Common.notify(cannotDeletePro, notification, JOptionPane.WARNING_MESSAGE);
                        refreshProduct();
                    } else {
                        if (date.plusDays(3).compareTo(today) == 0 || date.plusDays(3).isAfter(today)) {
                            int click = JOptionPane.showConfirmDialog(null, deletePro, notification, 2);
                            ...
                            Common.notify(timePro, notification, JOptionPane.WARNING_MESSAGE);
                            refreshProduct();
                        }
                    }
                }
            } else {
                Common.notify(selectPro, notification, JOptionPane.WARNING_MESSAGE);
            }
        } else {
            Common.notify(selectOrder, notification, JOptionPane.WARNING_MESSAGE);
        }
            ...
            if (orderDetail != null) {
                if (date.compareTo(today) == 0) {
                    if (checkProductID(orderDetail.getProductID())) {
                        Common.notify(notPro, notification, 1);
                        refreshProduct();
                        ...
                        if (b) {
                            Common.notify(updatedPro, notification, 1);
                            refreshProduct();
                        } else {
                            Common.notify(notUpdatePro, notification, JOptionPane.ERROR_MESSAGE);
                        }
                        refreshProduct();
                    }
                } else {
                    if (chkPromotion.isSelected()) {
                        Common.notify(cannotUpdatePro, notification, JOptionPane.WARNING_MESSAGE);
                        refreshProduct();
                    } else {
                        if (date.plusDays(3).compareTo(today) == 0 || date.plusDays(3).isAfter(today)) {
        ...
        if (order != null) {
            ...
            if (date.compareTo(today) == 0) {
                if (txtProductName.getText().trim().equals("")) {
                    Common.notify(selectInfoPro, notification, 1);
                } else if (!checkProductID(product.getProductID())) {
                    Common.notify(existPro, notification, 1);
                    refreshProduct();
                } else if (!"".equals(checkAddProduct())) {
                    Common.notify(checkAddProduct(), notification, 1);
                } else {
                    ...
                    if (b) {
                        Common.notify(addedPro, notification, 1);
                        refreshProduct();
                    } else {
                        Common.notify(notAddPro, notification, JOptionPane.ERROR_MESSAGE);
                    }
                    refreshProduct();
                }
            } else {
                if (chkPromotion.isSelected()) {
                    Common.notify(cannotAddPro, notification, JOptionPane.WARNING_MESSAGE);
                    refreshProduct();
                } else {
                    if (date.plusDays(3).compareTo(today) == 0 || date.plusDays(3).isAfter(today)) {
                        if (txtProductName.getText().trim().equals("")) {
...
//        }
    ...
    private void btnAddOrderActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddOrderActionPerformed
        if (txtCustomerName.getText().trim().equals("")) {
            Common.notify(selectCus, notification, 1);
            ...
            if (!"".equals(checkAddOrder())) {
                ...
                if (b) {
                    Common.notify(addedOrder, notification, 1);
                    getOrders(Dao_Orders.getOrders());
                    refresh();
                } else {
                    Common.notify(notAddOrder, notification, JOptionPane.ERROR_MESSAGE);
                }
            }
        }
    ...
    private void btnUpdateOrderActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnUpdateOrderActionPerformed
        ...
        if (order != null) {
            if (date.plusDays(3).compareTo(today) == 0 || date.plusDays(3).isAfter(today)) {
                ...
                if (b) {
                    Common.notify(updatedOrder, notification, 1);
                    getOrders(Dao_Orders.getOrders());
                    refresh();
                } else {
                    Common.notify(notUpdateOrder, notification, JOptionPane.ERROR_MESSAGE);
                }
            } else {
                Common.notify(timePro, notification, JOptionPane.WARNING_MESSAGE);
                refresh();
    ...
    private void btnDeleteOrderActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteOrderActionPerformed
        ...
        if (order != null) {
            if (date.plusDays(3).compareTo(today) == 0 || date.plusDays(3).isAfter(today)) {
                int click = JOptionPane.showConfirmDialog(null, deleteOrder, notification, 2);
                if (click == JOptionPane.YES_OPTION) {
                    boolean b = Dao_Orders.deleteOrder(order.getOrderID());
                    boolean p = returnProduct();
                    if (b & p) {
                        Common.notify(deletedOrder, notification, 1);
                        getOrders(Dao_Orders.getOrders());
                        refresh();
                    } else {
                        Common.notify(notDeleteOrder, notification, JOptionPane.ERROR_MESSAGE);
                    }
                }
            } else {
                Common.notify(timePro, notification, JOptionPane.WARNING_MESSAGE);
